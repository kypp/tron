...
const float PI = 3.14159265358979323846;	//liczba pi, potrzebna do obliczeń skręcania
const float FPS				= 60;			//liczba pożądanych klatek na sekundę
const float d_promien		= 4;			//domyślna grubość śladu zostawianego przez graczy
const float d_velocity		= 2;			//domyślna prędkość pojazdu graczy
const float d_ang_velocity	= 2;			//domyślna prędkość obrotu pojazdu	
...
{	
	//zmienne logiczne używane do obsługi zdarzeń i głównej pętli:
	bool redraw = true;
	bool doexit = false;	
	bool pocz   = true;
	...
	//deklaracja początkowych ustawień i kątów graczy:
	...
	//zmienne obsługujące przerwy w śladzie:
	bool przer = false;
	bool napraw = false;
	int  n	   = 0;
	...
	//tworzenie kolejki zdarzeń oraz timera zapewniającego odpowiednią częstotliwość wyświetlania, oraz bitmapy potrzebnej do wyświetlania:
	ALLEGRO_TIMER       *timer		 = al_create_timer(1.0 / FPS);
	ALLEGRO_EVENT_QUEUE *event_queue = al_create_event_queue();
	ALLEGRO_BITMAP		*bufor		 = al_create_bitmap(maxx, maxy);	
	
	//podpięcie do kolejki zdarzeń związanych z klawaturą, myszą, timerem i displayem:
	al_register_event_source(event_queue, al_get_display_event_source(display));
	al_register_event_source(event_queue, al_get_timer_event_source(timer));	
	al_register_event_source(event_queue, al_get_keyboard_event_source());
	al_register_event_source(event_queue, al_get_mouse_event_source());

	//wstępne wyczyszczenie ekranu i bufora - nie jest on czyszczony podczas rozgrywki:
	...
	while(!doexit)
	{
		...
		//instrukcja wykonywana FPS razy na sekundę, obsługująca wykonane przez graczy naciśnięcia klawiszy:
		...
		//obsługa żądania wyłączenia programu:
		else if(ev.type == ALLEGRO_EVENT_DISPLAY_CLOSE) {
			stan =-1;
			break;
		}
		//sprawdzanie naciśnięć klawiszy:
		...
		//sprawdzanie podniesień klawiszy:
		...
		if(redraw && al_is_event_queue_empty(event_queue) )
		{
			...
			//blokuję obecny stan buforu w celu szybkiego pobierania pikseli, bez blokowania al_get_pixel wykonuje się bardzo wolno, z nim mam stałe 240 FPS
			al_lock_bitmap(bufor, al_get_bitmap_format(bufor), ALLEGRO_LOCK_READONLY);

			//cała logika gry - wykonywana jeśli nie oczekuje się za zakończenie:
			if (!wait_for_input) {

				//wykrywanie kolizji oraz ich obsługa:				
				for (int i=0; i<4; i++)
				{
					if (still_alive[i]) {
			...
			//oczekiwanie chwilę w początkowej pozycji
			if (pocz) 
				{
					pocz = false;
					al_rest(0.2);
...
//zakładam, że fonty, prymitywy, klawiatura, mysz i samo allegro są zainicjalizowane
{
	//zmienne logiczne używane do obsługi zdarzeń i głównej pętli:
	bool redraw = true;
	bool doexit = false;	
	...
	//tworzenie kolejki zdarzeń oraz timera zapewniającego odpowiednią częstotliwość wyświetlania:
	ALLEGRO_TIMER       *timer		 = al_create_timer(1.0 / FPS);
	ALLEGRO_EVENT_QUEUE *event_queue = al_create_event_queue();
...
int wyniki(ALLEGRO_DISPLAY *display, parametry *params)
{
	...
	al_rest(0.2);
	//oczekiwanie na naciśnięcie klawisza ENTER albo SPACJI, lub ESC, sprawdzając klawiaturę co 1/FPS sekundy
...
int wstep(ALLEGRO_DISPLAY *display, parametry *params)
{
	...
	//ustalenie parametrow na domyslne, jeśli jeszcze nie zostały ustawione:
	...
	//podpięcie do kolejki zdarzeń związanych z klawaturą, timerem i displayem:
	al_register_event_source(event_queue, al_get_display_event_source(display));
	al_register_event_source(event_queue, al_get_timer_event_source(timer));	
	al_register_event_source(event_queue, al_get_keyboard_event_source());
...
{		
	...
	//inicjalizacja wszystkich potrzebnych modułów
	al_init();						
	al_init_image_addon();			
	...
	if (plik==NULL)
	{
		//ładowanie domyślnych ustawień i tworzenie pliku w wypadku niepowodzenia
