...
void rysuj_wybierz_graczy(ALLEGRO_DISPLAY *display, ALLEGRO_FONT *font, parametry *params)
{
	...
	//rysowanie kwadratów jeœli gracz jest wybrany:
	for (int i=0; i<4; i++)
		if (params->gracze[i])
...
void rysuj_wyniki(ALLEGRO_DISPLAY *display, ALLEGRO_FONT *font, parametry *params)
{
	...
	//wyœwietlenie wyników graczy bior¹cych udzia³ w grze:
	al_clear_to_color(al_map_rgb(0, 0, 0));
	char str[5];
	for (int i=0; i<4;i++)
		if (params->gracze[i])
